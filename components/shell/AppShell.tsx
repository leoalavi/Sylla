...
function BrandMark() {
  return (
    <Link href="/" className="flex items-center gap-2.5 px-1">
      <span
        aria-hidden
        className="flex h-8 w-8 items-center justify-center rounded-xl bg-primary text-sm font-bold text-white"
      >
        S
      </span>
...
function AccountSection() {
  ...
  if (!session.ready) {
    return <div className="h-9 animate-pulse rounded-xl bg-muted" />;
  }
  if (session.isSignedIn) {
    return (
      ...
      >
        <UserCircle2 size={18} className="shrink-0 text-primary" aria-hidden />
        <span className="min-w-0">
          <span className="block text-[11px] font-medium">Connected to Syllabus Sync</span>
          <span className="block truncate text-[11px] text-faint">{session.email}</span>
        </span>
      </div>
    );
  }
  return (
    <a
      href={getSignInUrl()}
      className="flex items-center gap-2 rounded-xl border border-border px-3 py-2 text-xs font-medium transition-colors hover:border-primary/60 hover:text-primary "
    >
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex h-dvh">
      ...
      {/* Desktop sidebar */}
      <aside className="hidden h-full w-60 shrink-0 flex-col gap-4 overflow-y-auto border-r border-border/60 bg-card/60 p-4 lg:flex">
        <BrandMark />
        <Link
          href="/chat"
          onClick={() => bumpNewChat()}
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-primary-hover"
        >
 ...
 : 'text-muted-foreground hover:bg-hover'
      ...
      <div className="flex h-full min-w-0 flex-1 flex-col">
        {/* Mobile header */}
        <header className="sticky top-0 z-40 flex items-center justify-between gap-3 border-b border-border/60 bg-[var(--background)]/90 px-4 py-2.5 backdrop-blur lg:hidden">
          <BrandMark />
          <div className="w-44">
        ...
        <nav
          aria-label="Main"
          className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-5 border-t border-border/60 bg-[var(--background)]/95 backdrop-blur lg:hidden"
        >
          {NAV_ITEMS.map(({ href, label, icon: Icon }) => {
            const active = isActive(pathname, href);
            return (
